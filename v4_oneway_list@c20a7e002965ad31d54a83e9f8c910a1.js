...
var DomesticOnewayDataAnalyzer = new function() {
    function p(t) {
        ...
        $jex.foreach(t, function(t, f, c) {
            ...
            if (h) {
                ...
                var p = OnewayFlightEntity.tryCreate(c, r, i, s);
                if (p) {
                    HotSale.setMinLate(p);
                    l.call(p);
                    $jex.event.binding(p, "updating", function() {
                        switch (this.type) {
                            case "oneway":
                                e.lowestOneway(this);
                                break;
                    ...
                    e.cabinType().length < 2 && e.cabinType(p.getCabinType());
                    n.addItem(c, p)
                }
                o = true
            }
    ...
    function T(t) {
        ...
        C(t);
        N(t.priceInfo, t.roundPriceInfo);
    ...
    function H() {
        var t = new FindTicketRec;
        $jex.event.trigger(e, "findTicketRec");
        if (n.getRecordCount() == 0) {
    ...
    this.setSearchService = function(n) {
        if (t) return;
        t = n;
        $jex.event.binding(t, "interSearch", v);
        $jex.event.binding(t, "validQuery", g);
        $jex.event.binding(t, "invalidQuery", y);
        $jex.event.binding(t, "loadedLongwell", x);
        $jex.event.binding(t, "loadedFirstData", b);
        $jex.event.binding(t, "loadedOnewayData", T);
        $jex.event.binding(t, "loadedTransfer", k);
        $jex.event.binding(t, "loadedExtInfo", L);
        $jex.event.binding(t, "loadedAVData", A);
        $jex.event.binding(t, "parsingFlightPriceData", O);
        $jex.event.binding(t, "searchEnd", H);
        $jex.event.binding(t, "onerror", j);
        $jex.event.binding(t, "pastLessSecond", D);
        $jex.event.binding(t, "zyfLoaded", P);
        $jex.event.binding(t, "ipBlock", m);
        $jex.event.binding(t, "getQueryId", _);
        $jex.event.binding(t, "loadedGroupinfo", w);
    ...
    this._initial = function() {
        ...
        if (typeof FlightListUISorter != "undefined") {
            $jex.event.binding(n, "refreshCurrentPage", function(e, t, n, r) {
                FlightListUISorter.resort(e, t, n, r)
            })
        }
    };
    var l = function() {
...
var DomesticOnewaySearchService = new function() {
    ...
    this.queryNext = function() {
        ...
        if (this.getTask()) {
            ...
            t._invoke_AVData()
        } else if (a != 2 && (u == 2 || u != 2 && m.length >= 2)) {
            $jex.console.info("queryNext:处理联程", " transferSearchState:", a, " isValidQuery:", v.isValidQuery, " onewayDatasLength:", m.length);
            t._invoke_transfer()
        } else {
    ...
    this._invoke_longwell = function() {
        ...
        $jex.ajax(h, c, function(n) {
            ...
            v = n;
            window.codeshare_rank_weight = v.codeshare_rank_weight || 0;
            t.queryId(n.queryID);
            E.queryID = n.queryID;
            E.serverIP = n.serverIP;
            var r = n.validate;
            if (r) {
    ...
    this._invoke_flightPriceData = function(r, i, s) {
        ...
        var v = setTimeout(function() {
            if (d) {
                ...
                e.src = "http://log.flight.qunar.com/l.gif?s=flight&p=onewayList&r=groupInfoError&info=" + encodeURIComponent(r) + "&msg=timeout&queryId=" + encodeURIComponent(E.queryID) + "&duration=" + encodeURIComponent(t)
            }
        }, 1e4);
        $jex.ajax(l, E, function(n) {
            d = 0;
            clearTimeout(v);
...
$jex.exec(function() {
        ...
        function i(e) {
            for (var i = 0; i < e.length - 1; i++) {
                for (var s = 0; s < e.length - 1 - i; s++) {
                    var o = e[s];
                    var u = e[s + 1];
                    if (t[o].lowestPrice() !== t[u].lowestPrice()) {
                        continue
                    }
                    ...
                    var h = 0;
                    var p = 0;
                    h = r && a.isCodeShare() ? 0 : c;
                    p = r && f.isCodeShare() ? 0 : c;
                    var d = n(a.deptTime());
                    var v = n(f.deptTime());
                    if (d > v) {
                        p += l
    ...
    FlightListUISorter.resort = function(e, t, n, i) {
        var s = 999999;
        ...
        var a = function(e) {
            if (t[e] != null) {
                ...
                var r = t[e];
                if (n != null) {
                    if (r.isCodeShare && r.isCodeShare()) {
        ...
        var N;
        var C;
        var k = ConfigManager.getConfig("PayCarrierSort");
        for (var L in k) {
        ...
        B = B.concat(A);
        if (P.length < 15) {
            H = 15 - P.length - A.length;
            B = B.concat(D.splice(0, H))
        }
        var j = [];
        var F = [];
        for (var g = 0, I = B.length; g < I; g++) {
            if (t[B[g]].type != "transfer" && t[B[g]].type != "compose") {
                F.push(B[g])
            } else {
                j.push(B[g])
            }
        ...
        o = o.concat(P, D, p);
        e.splice(0, e.length);
        $jex.foreach(o, function(t) {
            e.push(t)
        });
        $jex.event.trigger(FlightListUISorter, "dosort", e, t);
...
SearchStatusbar.prototype.updateStatus = function() {
    if (this.vendorMap.size() == 0) return;
    this.clear();
...
(function(e) {
    var t = $jex.$("dflightRecommendPanel");
    $jex.removeClassName(t, "m-nearline-rec");
    $jex.removeClassName(t, "m-find-ticket");
    var n = function() {
        var e = this;
        this.onsaleOneway = 0;
        ...
        $jex.event.binding(System.analyzer, "findTicketRec", function() {
            var t = System.analyzer.getData();
            var n = System.analyzer.onewayInfoMgr();
            var r = System.analyzer.lowestOneway();
            e.onsaleOneway = 0, e.onsaleEconomy = 0, e.totalOneway = 0;
            for (var i in t) {
                var s = t[i];
                if (s.type == "oneway") {
                    e.totalOneway++;
                    ...
                    if (o && o != 1e5) {
                        e.onsaleOneway++;
                        if (s.lowprInfo && s.lowprInfo.tc == "经济舱") {
                            e.onsaleEconomy++
                        }
                    }
                }
            ...
            var u = false;
            if (r && r.hasDiscount) {
                r.hasDiscount() ? u = false : u = true
            }
            if (e.onsaleOneway == 0 && e.totalOneway) {
                e.render("此航线已无直达票哦")
            } else if (e.onsaleEconomy == 0 && e.onsaleOneway) {
                e.render("此航线已无经济舱直达票哦")
            } else if (u) {
                e.render("此航线只有加价票哦")
            }
        });
        this.render = function(e) {
            var t = "http://subscribe.qunar.com/qunar/flight/ticketfinder/order/filling";
            var n = [];
            n.push("dptCity=" + encodeURIComponent(System.param.searchDepartureAirport));
            n.push("arrCity=" + encodeURIComponent(System.param.searchArrivalAirport));
            n.push("dptCityCode=" + encodeURIComponent(System.param.fromCode));
            n.push("arrCityCode=" + encodeURIComponent(System.param.toCode));
            n.push("dptDate=" + encodeURIComponent(System.param.searchDepartureTime));
            n.push("arrDate=" + encodeURIComponent(System.param.searchArrivalTime));
            t = t + "?" + n.join("&");
            var r = [];
            r.push('<div class="split-line"></div>');
            r.push('<div class="m-find-ticket-inner clrfix">');
            r.push('<div class="ico"></div>');
            r.push('<div class="tl">没有合适的票？去哪儿帮你找！</div>');
            r.push('<div class="msg"><span>', e, "</span></div>");
            r.push('<div class="btn"><a id="findtickbtn" class="link" target="_blank">申请找票</a></div>');
            r.push("</div>");
            var i = this.nearLineWrap.innerHTML;
            var s = i + r.join("");
            this.nearLineWrap.innerHTML = s;
...
flightResultController.prototype.initUI = function() {
    ...
    this.trackFilters = function(t) {
        ...
        var r = [];
        var i = [];
        var s = this.filterGroup;
        $jex.foreach(["起飞时间", "机型", "航空公司", "起飞机场", "降落机场", "舱位", "方式"], function(e, t) {
            var n = s.getFilterUI(e);
            if (n) r.push(n);
            i[t] = n ? 1 : 0
        });
        var o = e.getCheckGrpArr(r);
        if (o.length > 0) {
            n = o.join("^")
        }
...
flightResultController.prototype.bindUI = function() {
    ...
    $jex.event.binding(e.analyzer, "dataComplete", function() {
        ...
        setTimeout(function() {
            ...
            if (!e.endMemorier) {
                $jex.event.trigger(i, "afterDataLoad");
    ...
    TsinghuaOneWayTracker.setTimerToSaveTrack = function() {
        ...
        u = setTimeout(function() {
            if (s) {
                System.service.genTraceTimeStamp();
                System.analyzer.triggerTrace = true;
                TsinghuaOneWayTracker.trackOnRefreshed(s);
                e.trackFilters();
    ...
    TsinghuaOneWayTracker.traceFlightList = function() {
        if (System.analyzer.triggerTrace && s) {
...
(function() {
    ...
    function p(e) {
        var t = e;
        var n = window.SERVER_TIME.getTime();
        var r = (new Date("2015/5/29")).getTime();
        var i = $jex.$("js_flight_notice");
        var s = $jex.$("js_flight_notice_box");
        var o = t.searchDepartureAirport;
        var u = t.searchArrivalAirport;
        if ((o == "烟台" || o == "YNT" || u == "烟台" || u == "YNT") && n < r && i && s) {
            $jex.removeClassName(s, "hide");
            i.innerHTML = "烟台莱山国际机场自2015年5月28日0点起停止航班起降，将全部转至烟台蓬莱国际机场，请您注意机场变更，合理安排出行计划。"
        }
        i && i.style && (i.style.textIndent = "0px")
    }
    ...
    var v = function() {
        ...
        setTimeout(function() {
            ...
            var r = document.getElementById("hotel_hongbao");
            n.src = "http://simg1.qunarzz.com/site/images/flight/flight_search/hotel_hongbao_v1.jpg";
            n.onload = function() {
                r.appendChild(n);
            ...
            AD_Manage.qde_query = function(t) {
                var n = ["&to=", o.arrivalAirport.en, "&from=", o.departureAirport.en, "&cnkey=", encodeURIComponent(o.departureAirport.zh), "&s=", encodeURIComponent(e.searchDepartureAirport), "&s1=", encodeURIComponent(e.searchArrivalAirport), "&fromDate=", e.searchDepartureTime, "&st=oneway", "&pt=dmst"].join("");
                t(n)
            };
            $OTALOGIC.init(e.searchDepartureAirport, e.searchArrivalAirport, e.searchDepartureTime);
            $OTALOGIC.load_top("ifrmVendorBanner");
            $OTALOGIC.load_right();
            travelRec && travelRec.load(e.searchDepartureAirport, e.searchArrivalAirport, e.searchDepartureTime, e.searchArrivalAirport, "oneway");
            AD_Manage.load();
