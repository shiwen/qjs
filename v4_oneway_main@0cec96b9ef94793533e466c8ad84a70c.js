...
function DateLayer(e, t) {
    ...
    var H = function(n, r, p, d, v) {
        ...
        $jex.array.each([0, 1], function(e, r) {
            ...
            var v = i.getFullYear();
            var m = new Date(v, i.getMonth(), 0);
            var g = new Date(v, i.getMonth(), 1);
            var w = new Date(v, i.getMonth() + 1, 1);
            var E = (new Date(v, p - 1, 1)).getDay() - 1;
            if (E < 0) E = 6;
            var S = (new Date(v, p, 0)).getDate();
            var x = r == 0 ? f.getTime() <= m.getTime() : w.getTime() <= l.getTime();
            s.text('<div class="m-part">');
            s.text("<h3>");
            s.append("<a ", "a" + b++).text(' class="', r ? "downTd" : "upTd", '" ym="', QunarDate.format(r ? g : m), '" style="', x ? "display:block" : "display:none", '" href="javascript:;"></a>', v, "年", p, "月</h3>");
            ...
            for (var k = 0; k < 42; k++) {
                ...
                if (k < E) {
                    ...
                    if (!!h[O]) {
                        u[u.length] = O;
                        s.append("<td ", y++).text(' value="', O, '" data-pos="', e, '" class=" day_sel_area  ', C, '" >', L, "</td>")
                    } else {
                        s.append("<td ", y++).text(' value="', O, '" data-pos="', e, '" class="', C, '" >', L, "</td>")
                    }
        ...
        if (t.isInter) {
            ...
            var w = m == "1周之内" && !i ? QunarDate.getFuzzyDateText1() : QunarDate.getFuzzyDateText0();
            $jex.array.each(w, function(e, t) {
                ...
                s.append("<li ", y++).text(' value="', n.value, '" start="', n.start, '" end="', n.end, '" class="', _class, '">', e, "</li>")
            });
            s.text(" </ul>");
            s.text("</div>")
        }
...
function XCombox(e, t) {
    ...
    if (t.attrs)
        for (var u in t.attrs) this[u] = t.attrs[u];
    if (t.button) {
        if (t.button.mousedown) $jex.event.add(this, "buttonmousedown", t.button.mousedown)
    }
    if (t.input) {
        if (t.input.click) $jex.event.bindDom(e, "click", this, t.input.click);
        if (t.input.mousedown) $jex.event.bindDom(e, "mousedown", this, t.input.mousedown);
        if (t.input.change) $jex.event.add(this, "valuechange", t.input.change);
        if (t.input.keypress) $jex.event.bindDom(e, $jex.ie || $jex.safari ? "keydown" : "keypress", this, t.input.keypress)
    }
    FocusChecker(n, this, e);
    if (t.focus) $jex.event.add(this, "focus", t.focus);
    if (t.blur) $jex.event.add(this, "blur", t.blur);
    ...
    if (t.popups) {
        for (var u in t.popups) this.popups.createPopup(u, t.popups[u]).own = this
    }
    $jex.ie && $jex.event.bind(e, "beforedeactivate", function(e) {
        if ($jex.ie < 9 && this._f_leave) $jex.stopEvent(e);
        this._f_leave = 0
    });
...
function XPopup(e) {
    ...
    if (e.close) $jex.event.add(this, "close", e.close);
    if (e.open) $jex.event.add(this, "open", e.open);
    if (e.initialize) this.initialize = e.initialize
}
...
function FlightCityXCombox(e, t, n) {
    ...
    FlightCityXCombox.superclass.constructor.call(this, e, {
            ...
            change: function(e, n, r) {
                ...
                if (!r) {
                    ...
                    if (e) {
                        ...
                        u.cancel();
                        if (this.cache[e]) i.suggLoaded(this.cache[e]);
                        else {
        ...
        attrs: {
            setTip: function() {
                if (this.getValue() == "") this.setInfo(this.info || "城市名", "infotext");
                else this.setInfo("")
            },
            clear: function() {
                var e = this.popups.get("suggest");
        ...
        popups: {
            main: {
                ...
                open: function() {
                    ...
                    if (r.fromDateBox) {
                        if (!e["pos"]) e["pos"] = r["fromDateBox"]["pos"];
...
function SearchBox(e, t) {
    ...
    function d() {
        if (i.searchType == "deal") return false;
        var e = false;
...
DataSet.prototype._refreshDisplay = function(e) {
    ...
    $jex.event.trigger(this, "refreshCurrentPage", r, this._data, this.currentPage, this._pageSize);
    var i = [];
...
var QunarDate = $jex.exec(function() {
    ...
    var o = null;
    var u = null;
    var a = ["1周之内", "1个月内", "3个月内", "半年内", "1年", "圣诞", "元旦", "春节", "寒假", "清明", "五一", "暑假", "端午", "中秋", "国庆"];
    var f = ["+1周", "+2周", "1个月内", "3个月内", "1年"];
    var l = {
        "1周之内": {
            valid: true,
            value: "1周之内",
...
WrapperEntity.prototype.vendor = function() {
    ...
    if (typeof e == "undefined" || e == null) {
        ...
        e.dataSource(this.commInfoMgr().get("vendor", this.wrapperId()))
    }
...
WrapperEntity.prototype.isYoufei = function() {
    var e = this.lijian();
    return e && (this.isCsyf() || this.dataSource().youfei || this.dataSource().type === "uff" || false)
};
...
WrapperEntity.prototype.hasPickCar = function() {
    var e = this.vType() == "5" || this.vType() == "7" || this.vType() == "8" || this.vType() == "9" || this.vType() == "10";
    return e
};
WrapperEntity.prototype.giftInfo = function() {
    ...
    switch (e) {
        case 0:
            return "";
        case 1:
            n = '<p><font color="#CA8E38">1. 套餐包含<i class="rmb">&yen;</i>' + this.cPrice() + "接送机代金券" + (this.vAmount() || "1") + "张，可使用联系人手机号码登录去哪儿客户端-车车-接送用车使用</font></p>" + "<p>2. 凭代金券享受接机或送机服务时可直接抵扣对应金额，限去哪儿专车供应商，经济型、舒适型、商务型、豪华型等指定车型使用，具体适用车型可在去哪儿客户端-个人中心-代金券中查看，每次服务仅限使用1张代金券；</p>" + "<p>3. 支持北京、上海等多个指定城市使用，具体适用城市可在去哪儿客户端-个人中心-代金券中查看；</p>" + "<p>4. 自购买机票之日起30天内可用，详细使用规则请登录去哪儿客户端-车车查看；</p>";
            return n;
        case 2:
            switch (t) {
                case 9:
                    n = '<p class="tl">买保险赠超值礼包</p>' + '<p style="padding:3px 0 3px 3px"><strong>好礼一：</strong>去哪儿专车接送机代金券1张，直接抵扣<font color="#FF7301"><strong><i class="rmb">&yen;</i>' + this.vPrice() + "</strong></font></p>" + '<p class="ft"><a target="_blank" href="http://abc.trade.qunar.com/node/activity/voucherGift?voucherType=9&voucherPrice=' + this.vPrice() + '">查看详情</a></p>';
                    break;
                case 10:
                    n = '<p class="tl">买保险赠超值礼包</p>' + '<p style="padding:3px 0"><strong>好礼一：</strong>去哪儿专车豪华接送机代金券1张，直接抵扣<font color="#FF7301"><strong><i class="rmb">&yen;</i>' + this.vPrice() + "</strong></font></p>" + '<p class="ft"><a target="_blank" href="http://abc.trade.qunar.com/node/activity/voucherGift?voucherType=10&voucherPrice=' + this.vPrice() + '">查看详情</a></p>';
                    break
            }
            return n;
        default:
...
WrapperEntity.prototype._booking_url = function(e, t) {
    ...
    switch (this.ownerFlight().type) {
        case "roundtrip":
            n["isRt"] = 1;
            n["returnDepartureTime"] = this.ownerFlight().secondTrip().deptTime();
            n["returnArrivalTime"] = this.ownerFlight().secondTrip().arriTime();
            break;
    ...
    if (this.isRoundFlight()) {
        n["isRt"] = 1;
        n["returnDepartureTime"] = this.dataSource().flightInfo.secondtrip.dt;
        n["returnArrivalTime"] = this.dataSource().flightInfo.secondtrip.at
    }
    var r = window.location.param().from;
    if (r) {
        n["from"] = r
    }
    this._addEx_track(n);
    ...
    var i = this._transBu(n["prt"]);
    return "/booksystem/booking.jsp?" + i + "&" + $jex.toQueryString(n)
};
...
BookingLockScreenUI.prototype.preBooking = function(e, t) {
    ...
    this._vpr = 0;
    var n = this.entity,
        r = n.vType() !== undefined && !n.hasPickCar();
...
OnewayFlightVendorListUI.prototype.update = function(e) {
    ...
    this.onInit(function() {
        ...
        e._ad_timer = setTimeout(function() {
            ...
            if (n) n.innerHTML = '<iframe id="' + t + '" querystring="chan=flight&pg=list&pos=mid&site=qunar&size=728x90" scrolling="no" frameborder="0" height="0" width="100%" src="/site/adframe/ad.html#' + t + '#now"></iframe>'
        }, 100)
    })
};
...
OnewayFlightWrapperListUI.prototype.createWrapperUI = function(e, t, n) {
    ...
    if (t.vType() !== undefined && !t.hasPickCar()) {
...
OnewayFlightWrapperUI.prototype.insertVenderName = function(e) {
    ...
    var r = $jex.template('<div class="t-name"><%= vendorName %></div>');
    this.text(r(n))
};
...
OnewayFlightWrapperUI.prototype.requestTgq = function(e, t) {
    ...
    if ((n.isPriceKing() || n.isPgOta()) && n.tgqpr()) {
        p = n.tgqpr()
    }
    var v = this.find("tgq");
    var m = this.getDefaultTGQInfo(n);
    $jex.jsonp(o, {
...
OnewayFlightWrapperUI.prototype.getLabels = function(e) {
    ...
    if (t.specialCabinInfo()) {
        var o = t.specialCabinInfo();
        if (o.tipmsg == "") {
            o.tipmsg = o.icotext
        }
        var u = o.tipmsg && o.tipmsg.split("&");
        var a = "";
        $jex.each(u, function(e) {
            a += e + "<br/>"
        });
        i = s["specialCabin"];
        i.text = o.icotext;
        i.content = this._getTipHTML(a);
        r.push(i)
    } else if (t.isFCabin()) {
        i = s["firstClassCabin"];
        r.push(i)
...
OnewayFlightWrapperUI.prototype._getCarLabels = function(e, t) {
    ...
    var i = r["pickCar"];
    i.text = n.giftName();
    i.content = this._getTipHTML(n.giftInfo());
    e.push(i);
    return e
};
...
OnewayFlightWrapperUI.prototype.createLabelUI = function(e, t) {
    ...
    if (!e.length) {
        this.text('<span class="update-time">' + this.insert_UPDATETIME(n) + "</span>")
    } else {
        $jex.foreach(e, function(e, t) {
            var n = {
                color: e.color,
            ...
            r = $jex.template(['<a class="v-tag v-tag-<%= color%> <%= addClass%>" <%= hrefText%> id="<%= labelId %>">', '<div class="v-tag-tit">', "<span><%= text%></span>", "<%if(content){%>", '<i class="tag-tri"></i>', "<%}%>", "</div>", "<%if(content){%>", "<%= content%>", "<%}%>", "</a>"].join(""));
            i.text(r(n))
        })
    }
    this.text("</div>")
...
OnewayFlightWrapperUI.prototype.getAgeLimitInfo = function(e, t, n) {
    ...
    function f(e) {
        ...
        $jex.addClassName(s, t);
        r.innerHTML = n;
        $jex.element.show(s)
    }
    var r = n;
    var i = document.getElementById(e.labelId);
    var s = t.find("ageLimitPanel");
    var o = t.find("ageLimitContent");
    if (i) {
        var u = false;
        $jex.hover({
            act: i,
...
OnewayFlightWrapperUI.prototype.sortLabels = function(e) {
    e.sort(function(e, t) {
...
OnewayFlightWrapperUI.prototype.insert_insureInfo = function(e) {
    ...
    if (t.hasHongbao()) {
        ...
        i = "v-gift-normal"
    } else if (t.hasPickCar() && t.giftType() == 1) {
        var s = this.insert_carPrice(t);
        i = s >= 100 ? "v-gift-long" : s >= 10 ? "v-gift-normal" : "v-gift-short"
    } else if (n && r) {
        this.text('<div class="v-ins">');
        this.text("+", r, "保险");
        this.text("</div>");
        i = r >= 100 ? "v-gift-long" : r >= 10 ? "v-gift-normal" : "v-gift-short"
    } else {
        i = "v-gift-empty"
    }
    if (t.hasPickCar() && t.giftType() == 2) {
        this.insert_giftInfo(e, i)
    }
    this.text("</div>")
};
OnewayFlightWrapperUI.prototype.insert_giftInfo = function(e, t) {
    this.append("<div", "js-gift-icon", "");
    this.text(' class="p-tip-trigger v-gift ', t, '" >', '<span class="gift-ico">礼</span>', this._getTipHTML(e.giftInfo(), "js-gift-icon-tip"), "</div>")
};
...
OnewayFlightWrapperUI.prototype._buttonHTML = function(e, t, n) {
    ...
    if (n) {
        this.append("<a", n, ' data-evtDataId="' + this.newid("") + '" class="btn-book-org" href="#">' + r + "</span></a>")
    } else {
        this.text('<a class="btn-book-org" href="javascript:void(0)"><span><b>' + r + "</b></span></a>")
    }
    this.text("</div>")
};
OnewayFlightWrapperUI.prototype.logQrcode = function(e) {
    try {
        var t = e;
        var n = e.ownerFlight();
        var r = new Image;
        ...
        var u = n.arriCity().zh;
        var a = n.deptDate();
        (new Image).src = "http://log.flight.qunar.com/l.gif" + "?s=flight" + "&p=onewayList" + "&r=hongbaoQrcode" + "&flightCode=" + encodeURIComponent(s) + "&from=" + encodeURIComponent(o) + "&to=" + encodeURIComponent(u) + "&date=" + encodeURIComponent(a) + "&wrapperid=" + encodeURIComponent(i)
    } catch (t) {}
};
        ...
        if (n.hasPickCar() && n.giftType() == 1 || n.hasHongbao()) {
            $jex.event.bind(e, "mousemove", function(t) {
                ...
                while (!$jex.hasClassName(n, "qvt-column") && n != document) {
                    ...
                    if ($jex.hasClassName(n, "p-tip-trigger") || $jex.hasClassName(n, "e_qstar") || $jex.hasClassName(n, "v-tgq")) {
...
OnewayFlightWrapperUI.prototype.insert_carPrice = function(e) {
    ...
    this.text("套餐</i>");
    this.text("</span>");
    this.text(this._getTipHTML(['套餐总价<font color="#ff6600"><i class="rmb">&yen;</i>', u + s, '</font><br>套餐含<i class="rmb">&yen;</i>', n, o, i, '<i class="plus">+</i><i class="rmb">&yen;</i>', t, "保险"].join(""), "js-packge-price-tip"));
    this.text("</a>");
    return s
};
OnewayFlightWrapperUI.prototype.insert_hongbao = function(e) {
    ...
    o += '<p>用去哪儿APP扫码下单，买保险周边红包帮您每单抵现<i class="rmb">&yen;</i>' + i + "</p>";
    o += "<p><a>没有周边红包？扫码立即获取！</a></p>";
    o += "</div>";
...
FlagshipOnewayFlightWrapperUI.prototype.insert_vendorInfo = function(e) {
    ...
    if (n) {
        ...
        r && this.text('<img class="v-co-icon" src="http://simg1.qunarzz.com/site/images/new_airline_logo/small/' + r + '.png"/>');
        this.insertVenderName(t);
        this.insertStarInfo(t)
    }
    this.text("</div>")
...
FreeManOnewayFlightWrapperUI.prototype.getLabels = function(e) {
    var t = e;
    var n = t.fanxian();
    var r = FreeManOnewayFlightWrapperUI.superclass.getLabels.call(this, t);
    var i = ConfigManager.getConfig("OnewayListLabels");
    var s = [];
    var o = i["freeman"];
    o.content = this._getTipHTML("提供支付金额的超额发票");
    s.push(o);
    if (n) {
        o = i["lifan"];
        o.text = "支付立返<i class='rmb'>&yen;</i>" + n;
...
OtaOnewayFlightWrapperUI.prototype.insert_vendorInfo = function(e) {
    ...
    this.text(['<div class="v0">', '<a class="v-type-icon ', t, ' p-tip-trigger" ', $jex.ie == 6 ? 'href="javascript:;"' : "", ">", '<span class="ico">商旅优选</span>', this.getOtaTips(), "</a></div>"].join(""))
};
...
YouFeiOnewayFlightWrapperUI.prototype.insert_vendorInfo = function(e) {
    var t = this._getTipHTML(["<p>优飞币专享活动说明：</p>", "<p>1. 购买“优飞币专享”促机票产品，1优飞币可抵1元现金。</p>", "<p>* 如所拥有优飞币数量小于订单要求数量，则不可使用</p>", "<p>* 如该订单已赠送优飞币，则不可使用原有优飞币</p>", "<p>2. 如何获得优飞币？</p>", "<p>购买带有“送优飞币”标签的机票产品，即可获得与支付金额相等数量的优飞币。</p>", "<p>* 优飞币与订单联系人手机号绑定，有效期为自发币后一年内</p>", "<p>* 如使用优飞币抵扣现金购票，则不可获赠新的优飞币</p>"].join(""), "js-wrapper-logo-tip");
    var n = e.isYoufeiPg() ? "v-type-youfei-pg" : "v-type-youfei";
    this.text(['<div class="v0">', '<a class="v-type-icon ' + n + ' p-tip-trigger" ', $jex.ie == 6 ? 'href="javascript:;"' : "", ">", '<span class="ico">优飞币专享</span>', t, "</a></div>"].join(""))
};
...
YouFeiOnewayFlightWrapperUI.prototype.getTgqStr = function(e, t) {
    var n = e.tgqAdult || {};
    var r = n.tgqText || "" + "<br/><i>*</i>仅供参考,以订单标注的退改签规定为准。";
    return this.getTgqAfterFix(t, r)
};
...
YouFeiOnewayFlightWrapperUI.prototype.getLabels = function(e) {
    var t = e;
    var n = YouFeiOnewayFlightWrapperUI.superclass.getLabels.call(this, t);
    var r = [];
...
ZiyouxingOnewayFlightWrapperUI.prototype.insert_vendorInfo = function() {
    var e = this._getTipHTML("自由行套餐：机票+接送机服务", "js-wrapper-logo-tip");
    this.text(['<div class="v0">', '<a class="v-type-icon v-type-ziyouxing p-tip-trigger" ', $jex.ie == 6 ? 'href="javascript:;"' : "", ">", '<span class="ico">自由行</span>', e, "</a></div>"].join(""))
};
ZiyouxingOnewayFlightWrapperUI.prototype.getLabels = function(e) {
    var t = e;
    var n = FlagshipOnewayFlightWrapperUI.superclass.getLabels.call(this, t);
    var r = [];
...
TcabinOnewayFlightWrapperUI.prototype.insert_vendorInfo = function(e) {
    ...
    if (e.isPgPrivilege && e.isPgPrivilege()) {
        var r = "v-type-tcabin-pg"
    }
    var i = this._getTipHTML(["<p>1. 此产品参与低价特惠促销活动，退票或改签适用低价特惠促销退改签规则；<p>", "<p>2. 如低价特惠促销退改签规则不能满足您的需求，请选购非低价特惠产品或放弃低价特惠促销优惠" + n + "；<p>", "<p>3. 儿童票不参与低价特惠促销活动。<p>"].join(""), "js-wrapper-logo-tip");
    this.text(['<div class="v0">', '<a class="v-type-icon ', r, ' p-tip-trigger" ', $jex.ie == 6 ? 'href="javascript:;"' : "", ">", '<span class="ico">低价特惠</span>', i, "</a></div>"].join(""))
};
...
TcabinOnewayFlightWrapperUI.prototype.getTgqStr = function(e, t) {
    var n = e.tgqAdult || {};
    var r = n.tgqText || "" + "<br/><i>*</i>仅供参考,以订单标注的退改签规定为准。";
    return this.getTgqAfterFix(t, r)
};
...
TcabinOnewayFlightWrapperUI.prototype.getLabels = function(e) {
    var t = e;
    var n = t.vendor();
    var r = FlagshipOnewayFlightWrapperUI.superclass.getLabels.call(this, t);
    var i = [];
...
var QunarHistory = new function() {
    ...
    this.load = function() {
        ...
        if (e) {
            ...
            e.src = "http://history.qunar.com/history/newhistory.html"
        }
    };
...
XCombox.prototype.setInfo = function(e, t, n) {
    ...
    if (e) $jex.addClassName(this.wrappEl, "qbox_c");
    var r = "sinfo";
    if (t) r = r + " " + t;
...
XCombox.prototype.openMainMenu = function() {
    ...
    if (e && e.isOpend()) this.popups.close();
    else this.popups.open("main")
};
...
XCombox.prototype.timerListen = function(e) {
    if (e) {
        if (!this.listenID) this.listenID = setInterval($jex.callback(this, this._listenKey), 50)
    } else {
        if (this.listenID) {
...
XPopupManager.prototype.open = function(e) {
    ...
    if (t) {
        ...
        t.initialize();
        t.inited = true;
        if (this.current && this.current != t && this.current.isOpend()) this.current.close();
        if (!t.isOpend()) {
            t._open()
        }
...
XPopupManager.prototype.isOpend = function(e) {
    if (e) {
        if (this.popups[e]) return this.popups[e].isOpend()
    } else if (this.current != null) return this.current.isOpend();
    return false
};
...
FlightSuggestItemListLayer.prototype.moveCursor = function(e, t) {
    ...
    if (this.cursor == -1 && e == -1) {
        ...
        this.cursor += e;
        if (this.cursor < -1 || this.cursor >= this.nodes.length) this.cursor = -1
    }
    this.enter(this.cursor);
    if (t) this.select(this.cursor)
};
...
SearchSwitcher.prototype.active = function(e) {
    ...
    if (!t.forgetful) this.memories(this._oldtype || e);
    if (t.active) {
        t.active();
        this._count++
    }
...
TabGroup.prototype._initPanels = function() {
    if (!this.setting.panelContainerID) return;
...
window.searchTrack = function(e) {
    ...
    var i = null;
    var s = function(e) {
        var n = "/site/track.htm?action=" + (window._ba_utm_s || t) + "|" + e + "|&t=" + Date.parse(new Date);
        (new Image).src = n
    };
    o.prototype = {
        constructor: o,
