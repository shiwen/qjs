...
CACTI_monitoring.prototype._inList = function(e) {
    if ($jex.array.indexOf(this.timerList, e) > -1) return true;
    return false
};
CACTI_monitoring.prototype.start = function(e) {
    if (!this.isStart || !this._inList(e)) return;
...
FlightInfoManager.prototype.addPriceGroupDataSource = function(e, t) {
    ...
    for (var r in e) {
        ...
        if (n) this.addItem("lastPriceGroup", r, n, t)
    }
...
if (typeof QLib === "undefined") {
    ...
    var e = "/twell/searchrt_ui/ui_qunar_gsriiw.do";
...
var DomesticOnewayDataAnalyzer = new function() {
        ...
        if (e.roundPriceData) {
            ...
            if (e.roundPriceData.packagePriceData) {
                for (var r in e.roundPriceData.packagePriceData) {
                    ...
                    if (!e.priceData[e.flightCode]) e.priceData[e.flightCode] = {};
    ...
    function D(t, n, r) {
        ...
        if ($jex.array.indexOf(i, n) > -1) return;
        var s = 18;
        var o = e.infoMgr();
    ...
    this.lowestPrice = function() {
        ...
        var r = this.fuzzyFlight() == null ? Number.MAX_VALUE : function(e) {
            var t = e.fuzzyFlight().priceInfo || {},
                n, r;
            for (n in t) {
                r = (t[n] || {}).lowpr
            }
            return r ? r : Number.MAX_VALUE
        }(this);
        return Math.min(e, t, n, r)
    };
    this.lowestEntity = function() {
        var e = this.lowestTransfer() == null ? Number.MAX_VALUE : this.lowestTransfer().safeLowestPrice();
        var t = this.lowestOneway() == null ? Number.MAX_VALUE : this.lowestOneway().safeLowestPrice();
        var n = this.lowestCompose() == null ? Number.MAX_VALUE : this.lowestCompose().safeLowestPrice();
        if (e <= t && e <= n) return this.lowestTransfer();
        if (t <= e && t <= n) return this.lowestOneway();
        if (n <= e && n <= t) return this.lowestCompose()
    };
    var u = null;
...
var DomesticOnewaySearchService = new function() {
    ...
    this._invoke_flightPriceData = function(r, i, s) {
        ...
        S.wyf = u;
        var l = n + "/twell/flight/tags/onewayflight_groupinfo.jsp";
        var c = k;
        var h = s.priceInfo().d;
        var p = s.priceInfo().k;
        S.flightCode = r;
    ...
    this._invoke_ExtInfo = function() {
        ...
        var r = this.param;
        var i = n + "/twell/flight/DynamicFlightInfo.jsp";
        var s = {
            departureCity: r.fromCity,
            arrivalCity: r.toCity,
            departureDate: r.fromDate,
    ...
    this._invoke_AVData = function() {
        ...
        var r = n + "/twell/flight/OneWayFlight_Info.jsp";
...
var PriceCheckService = function() {
    ...
    var e = "/twell/flight/tags/onewayflight_pricecheck.jsp";
    var t = 1e4;
    var n = 1e3;
    var r = {};
    var h = {};
...
var TransferFlightEntity = function() {
    ...
    var u = null;
    this.arriDate = function() {
        if (typeof u == "undefined" || u == null) {
            u = this.secondTrip().arriDate()
        }
        return u
    };
    var a = null;
    this.arriTimeValue = function() {
        if (typeof a == "undefined" || a == null) {
            a = this.secondTrip().arriTimeValue()
        }
        return a
    };
    var f = null;
    this.transferAirport = function() {
        if (typeof f == "undefined" || f == null) {
            var e = this.firstTrip();
            var t = this.secondTrip();
            if (e.arriAirportCode() == t.deptAirportCode()) {
                f = e.arriAirport().ab
            } else {
                f = e.arriAirport().ab + "-" + t.deptAirport()
            }
        }
        return f
    };
    var l = null;
    this.transferTime = function() {
        if (typeof l == "undefined" || l == null) {
            var e = this.firstTrip();
            var t = this.secondTrip();
            var n = this.transferDisDays();
            l = e.arriTime() + "-" + t.deptTime() + (n ? "+" + n + "天" : "")
        }
        return l
    };
    var c = null;
    this.transferDisDays = function() {
        if (typeof c == "undefined" || c == null) {
            var e = this.firstTrip();
            var t = this.secondTrip();
            c = $jex.date.distance(e.arriDate().replace("-", "/"), t.deptDate().replace("-", "/"))
        }
        return c
    };
    var h = null;
    this.transferStopDur = function() {
        if (typeof h == "undefined" || h == null) {
            var e = this.firstTrip();
            var t = this.secondTrip();
            h = FlightUtil.duration((t.deptTimeValue() - e.arriTimeValue()) / (1e3 * 60))
        }
        return h
    };
    var p = null;
    this.distanceDays = function() {
        if (typeof p == "undefined" || p == null) {
            var e = this.firstTrip();
            var t = this.secondTrip();
            p = $jex.date.distance(e.deptDate().replace("-", "/"), t.arriDate().replace("-", "/"))
        }
        return p
    };
    var d = null;
    this.dur = function() {
        if (typeof d == "undefined" || d == null) {
            d = FlightUtil.duration((this.arriTimeValue() - this.deptTimeValue()) / (1e3 * 60))
        }
        return d
    };
    this.lowestPrice = function() {
...
SingleTripFlightWrapperEntity.prototype._booking = function(e, t, n) {
    t = t || {};
    if (!t.BookingLocation) {
        t.BookingLocation = "list_all"
    }
    var r = this._booking_url(e, t);
    var i = 1;
    t = t || {};
    if (t["prt"] === 0) {
        i = 2
    ...
    var s = this.ownerFlight().owner();
    if (s) {
        i += "&package=" + s.firstTrip().code() + "/" + s.secondTrip().code()
    }
    window.open(r);
    this.logBooking(n);
    $jex.event.trigger($jex.$("hdivResultPanel"), "fem_booking");
    this._bookingBtnTrace();
    TsinghuaOneWayTracker.track("btype", i, System.service.traceTimeStamp, null, "&burl=" + encodeURIComponent(r) + "&wt=" + System.service.wrapperExpandStamp);
    this._booking_track()
};
...
TransferFlightUI.prototype.update = function(e) {
    this.clear();
    this._homeNode = null;
    this.append("<div", "itemBar", ' class="avt-column avt-column-trans');
    if (this.state()) {
    ...
    this.text(this._getStaticUI(e));
    this.append('<div class="c6">&nbsp;</div>');
    this.append('<div class="c7">');
    this.text('<div class="a-price">');
    this.text(this.getPriceInfoHTML(e));
    this.text("</div>");
    this.insertBookingBtn(e);
    this.text("</div>");
    this.text("</div>");
...
TransferFlightUI.prototype._getStaticUI = function(e) {
    var t = [],
        n = e.firstTrip(),
        r = e.secondTrip();
    ...
    t.push(this._getOneFlightAirHTML(r));
    t.push("</div></div>");
    t.push("</div>");
    t.push('<div class="c2">');
    ...
    t.push('<div class="a-tm-be">' + e.dur() + "</div>");
    t.push('<a class="a-zh-cont p-tip-trigger"' + ($jex.ie == 6 ? ' href="javascript:;"' : "") + ">");
    var i = this._getZhInfoHtmlArr(e);
    t.push(i.tit.join(""));
    t.push(this._getTipHTML(i.cont.join("")));
    t.push("</a>");
    t.push("</div>");
    t.push("</div>");
    t.push('<div class="c4">');
    t.push(this._getArriHtml(r, this._getDisanceDaysHtml(e)));
    t.push("</div>");
    t.push('<div class="c5">');
    t.push('<div class="vlc-wp"><div class="vlc-ref"></div><div class="vlc-con">');
    t.push('<div class="a-pty">' + n.quasipointRateHTML() + "</div>");
    t.push('<div class="a-pty">' + r.quasipointRateHTML() + "</div>");
    t.push("</div></div>");
    t.push("</div>");
    return t.join("")
};
TransferFlightUI.prototype.getPriceInfoHTML = function(e) {
    var t = [];
    t.push('<div class="a-low-prc"><i class="rmb">&yen;</i>', Price_html_wide.getHTML(e.lowestPrice()), "</div>");
    t.push('<div class="a-prc-ex">', PriceUtil.getTransferDiscount(e.lowestDiscount()), "</div>");
    return t.join("")
};
TransferFlightUI.prototype._getZhInfoHtmlArr = function(e) {
    var t = e.firstTrip(),
        n = e.secondTrip();
    var r = [],
        i = {
    ...
    var s = this._getStopHtmlInfo(t);
    if (s && s.tit) {
        s.cont_tit = "第一段经停";
        r.push(s)
    }
    var o = this._getTransferHtmlInfo(e);
    if (o && o.tit) {
        o.cont_tit = "中转";
        r.push(o)
    }
    var u = this._getStopHtmlInfo(n);
    if (u && u.tit) {
        u.cont_tit = "第二段经停";
        r.push(u)
    }
    $jex.array.each(r, function(e) {
        i.tit.push(e.tit);
        i.cont.push('<p><span class="bold">' + e.cont_tit + "</span></p>" + e.cont)
    });
    return i
};
FlightUI.prototype._getTransferHtmlInfo = function(e) {
    var t = e.firstTrip(),
        n = e.secondTrip(),
        r = [];
    r.tit = '<div class="ifo"><i class="z">转</i>' + t.arriCity().zh + "</div>";
    r.cont = "" + "<p>中转城市：" + t.arriCity().zh + "</p>" + "<p>中转时间：" + e.transferTime() + "</p>" + "<p>中转机场：" + e.transferAirport() + "</p>" + "<p>停留时间：" + e.transferStopDur() + "</p>" + "<p>中转税费：每段航班均需缴纳税费</p>";
    return r
};
...
TransferFlightUI.prototype.insertBookingBtn = function(e) {
    ...
    this.append("<a", "openwrapperbtn", '  data-evtDataId="' + this.newid("") + '"   href="##" hidefocus="on" onfocus="this.blur();" title="点击查看订票网站" class="btn-book"><i class="ico-arr"></i>订&nbsp;&nbsp;票</a>');
    this.text("</div>")
};
...
$jex.extendClass(TransferFlightVendorListUI, VendorListUI);
TransferFlightVendorListUI.prototype._insertOneWrapper = function(e, t) {
    ...
    this.text("</div>");
    this.append("<h3 ", s, "");
    this.text(">第", t.msg, "段&nbsp;<span>", n.deptCity().zh, "&nbsp;-&nbsp;", n.arriCity().zh, "</span></h3>");
    this.text("</div>");
    this.text('<div id="' + this.newid("wrapperHd") + '" class="wrapper-list-header">');
    this.text("</div>");
    ...
    if (t.id == "transfer_p1") {
        this.onInit(function() {
            this.createHongbaoHtml(this.find("wrapperHd"))
        })
    }
    r = i = e = t = null
};
TransferFlightVendorListUI.prototype.update = function(e) {
    ...
    this.text('    <p>购买须知：两段需要分别购买，每段航班需分别缴纳税费，请确认两航班均有效再付款。详情查看《<a target="_blank" href="http://www.qunar.com/site/zh/Multi-city.shtml?', (new Date).getTime(), '">中转程机票购买须知</a>》</p>');
    this.text("</div>");
    var t = false,
        n = null;
    var r = {
    ...
    this.onInit(function() {
        ...
        e._ad_timer = setTimeout(function() {
            ...
            if (n) n.innerHTML = '<iframe id="' + t + '" querystring="chan=flight&pg=list&pos=mid&site=qunar&size=912x90" scrolling="no" frameborder="0" height="0" width="100%" src="/site/adframe/ad.html#' + t + '#now"></iframe>'
        }, 100)
    });
    n = r = e = null
};
...
var RoundTripFlightRecommend = new function(e) {
    ...
    this.render = function(e) {
        ...
        var i = ['<div class="b_fly_pmt">', '<div class="e_pmt_tit"><span class="city">' + e.fromCity + '<i class="i-city"></i>' + e.toCity + "</span><h3>往返一起买</h3></div>", '<div class="e_pmt_cont">', '<ul class="ul_wf_line" id="js_lproute">'];
        for (t = 0; t < r; t++) {
            var s = e.list[t].url;
            ...
            o.push('<div class="d_tm"><span class="s_day">' + e.list[t].fromDate + "去</span>");
            o.push('<span class="s_day">' + e.list[t].toDate + "回</span>");
            if (e.list[t].lowPrice) {
                o.push('<i class="lowst">低</i>')
            }
            o.push("</div>");
            o.push('<div class="d_pr">');
            o.push('<span class="lnk_bk">订票</span>');
            o.push('<span class="s_prc tjb">特价包&nbsp;<i class="rmb">¥</i>' + e.list[t].pPrice);
            o.push("</span>");
            o.push('<span class="s_prc zj"><span class="normal">机票直降</span>&nbsp;<i class="rmb">¥</i>' + (e.list[t].oPrice - e.list[t].pPrice) + "</span>");
            o.push("</div>");
            o.push("</a>");
            o.push("</li>");
            i.push(o.join(""))
        ...
        i.push('</ul></div><div class="e_pmt_ft"><a target="_blank" id="js_moreFlight" href=' + n + '>更多航班低价组合<i class="ftsong">&gt;&gt;</i></a></div></div>');
...
(function() {
    ...
    var SpringHotRoundtrip = new function() {
        ...
        var baseHtml = '<a class="prev-btn" href="javascript:;"><span class="btn-inner"><i class="g-ico g-ico-arrowl" id="arrowleft"></i></span></a>                        <div class="day-tab">                            <div  id="searchDateBar-nav" class="date-group">                                <ul class="ul_flt_date clrfix j-date-nuit"></ul>                            </div>                        </div>                        <a class="next-btn" href="javascript:;"><span class="btn-inner"><i class="g-ico g-ico-arrowr" id="arrowright"></i></span></a>                        <a class="prc_cld" href="javascript:;">                            <i class="m_prc_icon"></i>                            <p id="priceCd" class="m_pc" style="visibility: visible;">价格日历</p>                            <div id="priceCalendar" class="prCd"></div>                        </a>';
        ...
        this.template_sevenday = function(context, __onerror) {
            ...
            if (context.defined == null) context.defined = function(e) {
            ...
            try {
                (function(_OUT, _CONTEXT) {
                    with(_CONTEXT) {
                        ...
                        _OUT.write('<span id="pcDown"  ');
                        if (prevMonth) {
                            _OUT.write('class="prev" onclick="SpringHotRoundtrip.getPriceData(\'' + prevMonthDate + "')\"")
                        } else {
                            _OUT.write('class="prev disable-prev" style=";cursor:default;"')
                        }
                        _OUT.write('><i class="g-ico g-ico-arrowl"></i></span><span id="pcUp" ');
                        if (nextMonth) {
                            _OUT.write('class="next" onclick="SpringHotRoundtrip.getPriceData(\'' + nextMonthDate + "')\"")
                        } else {
                            _OUT.write(' class="next disable-next"  style="cursor:default;"')
                        }
                                    ...
                                    for (var col_index in __LIST__col) {
                                        ...
                                        if (dd) {
                                            ...
                                            if (pd != false) {
                                                ...
                                                _OUT.write("     ");
                                                _OUT.write(dd[3]);
                                                _OUT.write('<br/><span class="nopr-line">--</span>')
                                            }
                                            _OUT.write(" ")
                                        } else {
                                            _OUT.write("     <td>     &nbsp;")
        ...
        this.initScroller = function() {
            ...
            e.scroller.init({
                ...
                isHorizontal: true,
                step: parseInt(getStyleAttrVal(e.resultUnit, "width"), 10) || 721,
        ...
        this.roundtripVendor = function(e, t, n) {
            ...
            $jex.hash.each(t, function(t, o) {
                ...
                if (n[a]) {
                    if (e + parseFloat(u.price) > n[a].price) {
                        ...
                        u.bu = s.createUrl(u.dt, true) + "&from=tejia_recmd_pac", u.fromDate = $jex.date.format(r, "MM-dd");
            ...
            var o = [];
            for (var u in this._packageData) o.push(u);
            o.sort(function(e, t) {
        ...
        this.priceCalendar = function() {
            if (!this.priceCacheData) return;
            var e = this,
                t, n = [],
                r = $jex.date.parse(this._dateStr);
            if (!this._sdate) {
                this._sdate = r
            }
...
(function(e) {
    ...
    var r = new function() {
        ...
        this.load = function(e) {
            var n = e.from;
            r = n == "near_flight" || n == "near_airport" ? 0 : 1;
            if (!r) {
                return
            }
            var i = this,
                s = {
                    from: e.searchDepartureAirport,
                    to: e.searchArrivalAirport,
                    start_date: e.searchDepartureTime
                };
        ...
        this.render = function(t) {
            function c() {
                ...
                return '<div class="a-price">' + '<div class="a-low-prc"><i class="rmb">¥</i>' + Price_html_wide.getHTML(t.pr) + "</div>" + '<div class="a-prc-ex">起</div>' + "</div>"
            }

            function h() {
                return "/twell/flight/Search.jsp?fromCity=" + encodeURIComponent(r) + "&toCity=" + encodeURIComponent(i) + "&fromDate=" + s + "&searchType=OnewayFlight&from=near_flight"
            }
            if ((!t.trainpr || t.trainpr == 999999999) && !t.pr) {
                return false
            }
            var r = t.dc,
                i = t.ac,
                s = t.dt;
            var o = e.System.service.param,
                u = o.fromCity,
                a = o.toCity;
            var f = "";
            if (r == u) {
            ...
            var l = '<div class="m-nearline-rec-inner b-avt-lst clrfix">' + '<div class="c0">&nbsp;</div>' + '<div class="c1">' + '<div class="a-name">' + '<span class="ico"></span>' + '<span class="name-txt">邻近推荐</span>' + '<span class="tri"><div class="arr"><p class="arr-o">◆</p><p class="arr-i">◆</p></div></span>' + "</div>" + "</div>" + '<div class="c2-4">' + '<div class="a-line">' + '<span class="line-name">' + r + "-" + i + "</span>" + '<span class="line-ex">' + f + "</span>" + "</div>" + "</div>" + '<div class="c5">&nbsp;</div>' + '<div class="c6">' + c() + "</div>" + '<div class="c7"><a href="' + h() + '" class="a-link">查看详情</a></div>' + "</div>";
...
var BookingPriceCheck = function() {
    ...
    e.check = function(e, t) {
        var n = e.ownerFlight();
        if (n.type == "onewayInTransfer") return;
...
flightResultController.prototype.initUI = function() {
    function t() {
        $jex.event.trigger(e.fuzzyFlightUI, "doFilterFuzzyFlight");
    ...
    this.fuzzyFlightUI = new FuzzyFlightUI({
        ...
        on: {
            ...
            doFilterFuzzyFlight: function() {
                if (!this.data) {
                    return false
                }
                var t = (e.analyzer.pageInfo() || {
                    pageIndex: 0
                }).pageIndex;
                if (!t) {
                    this.hideVendorPanel();
                    var n = this.filterByFirm();
                    n && this.fillWrapper()
                } else {
    ...
    $jex.event.binding(this.service, "loadedFirstData", function() {
        var t = e.resultList;
        ScrollShow.count = 30;
        ScrollShow.init(t.elem(), 30, function() {
            $jex.element.show($jex.$("detailPage"));
            if (ScrollShow.num < 30) return;
            ScrollShow.count += 30;
            $jex.event.trigger(e.analyzer, "dataComplete");
            ScrollShow.loaded = false;
            if (ScrollShow.count >= ScrollShow.num) {
                ScrollShow.stop()
            }
        });
    ...
    this.sort_time_handler = new SortHandler({
        ...
        on: {
            clickSort: function(n) {
                ...
                $jex.$("btnPriceOrderArror").getElementsByTagName("i")[0].className = "i-arr-ud";
    ...
    this.sort_price_handler = new SortHandler({
        ...
        on: {
            clickSort: function(n) {
                ...
                $jex.$("btnDepttimeOrderArror").getElementsByTagName("i")[0].className = "i-arr-ud";
    ...
    this.timeoutMemorier = function(e) {
        function l() {
            ...
            if (i || s) {
                $jex.array.each([i, s], function(e, t) {
                    ...
                    if (i === "i-arr-ud") {
                        n[r] = 0
                    } else if (i === "i-arr-ud-up") {
                        n[r] = 1
                    } else if (i === "i-arr-ud-down") {
                        n[r] = 2
                    }
                })
            }
        ...
        var s = [r.filterGroup],
            o = r.resultList;
        filterGroup = r.filterGroup, nameHX = {
            "起飞时间": "dt",
            "机型": "pt",
            "航空公司": "hs",
            "起飞机场": "da",
            ...
            reSetByRecord: function() {
                ...
                if (n && a != "sort") {
                    $jex.foreach([n.pr, n.dt], function(e, t) {
                        ...
                        var n = t == 0 ? s : i;
                        if (e == 1) {
                            n.arrow.className = "i-arr-ud-up";
                            n.state(false)
                        } else {
                            n.arrow.className = "i-arr-ud-down";
                            n.state(true)
                        }
                        r.actionType = 2;
                        a = "sort";
                        $jex.event.trigger(n, "clickSort", [
                            [n._setting.sortKey, n.state()]
                        ])
                    })
                }
...
flightResultController.prototype.resetScrollShow = function() {
    ScrollShow.stop();
    ScrollShow.reset()
};
flightResultController.prototype.bindUI = function() {
    ...
    $jex.event.binding(e.analyzer, "dataComplete", function() {
        ...
        r.loadData(e.analyzer.resultData().slice(0, ScrollShow.count || 30), e.analyzer);
...
SortHandler.prototype._init = function() {
    ...
    $jex.event.binding(t, "click", function(t) {
        $jex.stopEvent(t);
        if (e.state()) {
            n.className = "i-arr-ud-up";
            e.state(false)
        } else {
            n.className = "i-arr-ud-down";
            e.state(true)
        }
        $jex.event.trigger(e, "onUserActied");
        $jex.event.trigger(e, "clickSort", [
            [e._setting.sortKey, e.state()]
        ]);
        $jex.log({
            s: "flight",
            p: "onewayList",
...
BookBtnTracker.prototype.send = function(e) {
    ...
    if (o || u) {
        $jex.array.each([o, u], function(e, t) {
            ...
            if (r === "i-arr-ud") {
                s[n] = 0
            } else if (r === "i-arr-ud-up") {
                s[n] = 1
            } else if (r === "i-arr-ud-down") {
                s[n] = 2
            }
        })
    }
    ...
    $jex.ajax("/rms/data/pickflight.json", p, function() {}, {
        onerror: t._onerror,
        method: "POST"
    })
};
...
(function() {
    function a(e) {
        flightSearchBox = new FlightSearchBox(e);
        flightSearchBox.init("#js_schwrap")
    }
    ...
    $(document).on("usertoolbarready", function(e, t) {
        var n = false;
        t.add([{
            id: "toolbox",
            name: "工具",
            ...
            onOpen: function(e) {
                if (n === false) {
                    n = true;
                    var r = document.cookie.indexOf("_t=") >= 0 && document.cookie.indexOf("_q=") >= 0;
                    var i, s;
                    if (r) {
                        i = "http://user.qunar.com/flight_toolbox.jsp?catalog=ownorders&from=flight_dibu_chupiao"
                    } else {
                        i = "http://user.qunar.com/order/query.jsp?ret=http%3A%2F%2Fuser.qunar.com%2Fflight_toolbox.jsp%3Fcatalog%3Downorders%26from%3Dflight_dibu_chupiao"
                    }
                    var o = {
                        ...
                        u = QunarHistory && QunarHistory.firstInter;
                    if (u) {
                        o.pfromCity = u.FromCity();
                        o.ptoCity = u.ToCity();
                        o.pfromDate = u.FromDate()
                    }
                    var a = o.pfromCity.match(/^(.*)\((\w+)\)$/),
                        f = o.ptoCity.match(/^(.*)\((\w+)\)$/);
                    var l = {
                        fromCity: a ? a[1] : o.pfromCity,
                        toCity: f ? f[1] : o.ptoCity,
                        fromDate: o.pfromDate,
                        fromCode: a ? a[2] : "",
                        toCode: f ? f[2] : "",
                        from: o.from,
                        isInter: true
                    };
                    s = "/site/pricetrend.htm?" + $jex.toQueryString(l);
                    t.fillPanel(e, "<div>" + '<ul class="e_fly_route clrfix">' + "<li>" + '<a href="' + i + '" target="_blank" action="cpzt">' + '<i class="ico_cpzt"></i>出票状态' + "</a>" + "</li>" + "<li>" + '<a href="http://subscribe.qunar.com/info/addSub.do?s=10" target="_blank" action="yyjp">' + '<i class="ico_tpgq"></i>预约机票' + "</a>" + "</li>" + "<li>" + '<a href="http://user.qunar.com/flight_toolbox.jsp?catalog=passenger" target="_blank" action="cjrxx">' + '<i class="ico_zxzj"></i>乘机人信息' + "</a>" + "</li>" + "<li>" + '<a href="http://help.qunar.com/category.html?id=12" target="_blank" action="jpgl">' + '<i class="ico_jjtx"></i>机票攻略' + "</a>" + "</li>" + "<li>" + '<a href="' + s + '" target="_blank" action="jgqs">' + '<i class="ico_hbdt"></i>价格趋势' + "</a>" + "</li>" + "<li>" + '<a href="http://user.qunar.com/flight_toolbox.jsp?catalog=searchvendors" target="_blank" action="cxdls">' + '<i class="ico_gjyy"></i>查询代理商' + "</a>" + "</li>" + "</ul>" + "</div>")
                }
            }
        }]);
        t.rightSide()
        ...
        i.check = function(e) {
            ...
            if ("oneway" === e.searchType) {
                ...
                if (r > t) {
                    ...
                    this.data.round = false;
                    this.data.departureDate = $jex.date.format(t);
                    this.data.drange = e.drange, this.data.search = e.search;
                    var s = d(e);
                    s.fromDate = this.data.departureDate;
                    this.data.href = c(s)
                }
